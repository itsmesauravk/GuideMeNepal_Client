...
const Sidebar = () => {
  ...
  // Add padding to content when using fixed sidebar in desktop view
  const DesktopSidebarPadding = () => (
    <div className={`hidden lg:block ${isCollapsed ? "w-16" : "w-96"}`}></div>
  )

  // Add padding to content when mobile top nav is present
  const MobileTopNavPadding = () => <div className="lg:hidden h-28"></div>
